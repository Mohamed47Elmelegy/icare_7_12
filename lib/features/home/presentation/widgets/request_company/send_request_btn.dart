    ...
    return BlocListener<BookingBloc, BookingState>(
      listener: (ctx, state) {
        ...
        if (state is SendBookingRequestFialedState) {
          SnackBarBuilder.showFeedBackMessage(
              context, state.msg.toString(), Colors.red);
          Navigator.of(ctx).pop();
        }
      },
      listenWhen: (previous, current) =>
          current is SendBookingRequestLoadingState ||
          current is SendBookingRequestFormSuccessfullyState ||
          current is SendBookingRequestFialedState,
      child: BlocBuilder<BookingBloc, BookingState>(
        builder: (ctx, state) {
          var bloc = BookingBloc.get(ctx);
          if (state is SendBookingRequestLoadingState) {
