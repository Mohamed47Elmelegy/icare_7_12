      ...
      children: [
        ...
        BlocBuilder<BookingBloc,BookingState>(
          builder: (ctx,state){
            ...
            var list = ['تغيير الضمادات','العناية بالقسطرة البولية','العناية بأنبوب التغذية',
            'إعطاء الأدوية عن طريق الحقن','جلسات علاج طبيعي','رعاية ما بعد العمليات الجراحية','رعاية مرضى الشيخوخة'];
            return GridView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                return  CheckboxListTile(
                  ...
                  title: CustomText(text: item, fontSize: AppStyle.small.sp-2),
                  value: item.trim() == bloc.moreNeed.trim(),
                  onChanged: (value)=> bloc.add(UpdateRequestFormDataEvent(data: {'more_need': item.toString().trim()})),
                  onFocusChange: (value)=> bloc.add(UpdateRequestFormDataEvent(data: {'more_need': item.toString().trim()})),
                );
              }
            );
          },
