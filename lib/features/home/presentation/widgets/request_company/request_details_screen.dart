...
class RequestDetailsScreen extends StatelessWidget {
  ...
  const RequestDetailsScreen({super.key,this.showActions = false,required this.id,required this.requestEntity});
        ...
        child:  SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              CustomText(text: translate('order.companie_offers'), fontSize: AppStyle.average.sp),
              if(requestEntity.requestOfferList.isEmpty)...[
                Container(
                  margin: const EdgeInsets.all(10),
                  alignment: Alignment.center,
                  child: CustomText(text: translate('order.no_offers'), fontSize: AppStyle.average.sp,)
                ),
              ]else
              ListView.separated(
                shrinkWrap: true,
                ...
                itemBuilder: (ctx,index){
                  ...
                  int acceptedIndex = requestEntity.requestOfferList.indexWhere((val)=> val.status=='accepted');
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          CustomText(
                            text: "${item.offerPrice}${translate('icare.le')}", 
                            fontSize: AppStyle.average.sp,
                          ),
                        ],
                      ),
                      ...
                      CustomButton(
                        ...
                        color: kWhite, 
                        onPressed: ()async {
                            final res = await CustomDialogs.sureToSubmit(context);
                           if(res){
                            final result = await OrderRemoteDataSource.acceptOffer(data: {
                              'offer_id':item.id,
                            });
                            if(!context.mounted)return;
                            if(result==true){
                              SnackBarBuilder.showFeedBackMessage(context, translate('toast.gmail_send'), kPrimary);
                              ...
                              SnackBarBuilder.showFeedBackMessage(context, result, Colors.red);
                            }
                           }
                        },
                      ),
