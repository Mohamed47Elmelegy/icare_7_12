...
class CategoriesBloc extends Bloc<CategoriesEvent, CategoriesState> {
  ...
  getAllSliders(emit) async {
    ...
    try {
      ...
      res.fold((l) {
        ...
        anotherSliders =
            data.where((element) => element.kind != "slider").toList();
        emit(FetchSliderSuccessfullyState());
      });
    } catch (e) {
  ...
  filterSliderByLang(List<SliderEntity> sliders) {
    if (Util.getLang() == "ar") {
      return sliders
          .where((element) =>
              element.title.toString().toLowerCase().contains("ar"))
          .toList();
    } else {
      return sliders
          .where((element) =>
              element.title.toString().toLowerCase().contains("en"))
          .toList();
    }
  }
  ...
  updateVideoController(UpdateVideoControllerEvent event, emit) {
    emit(FetchPublicationsLoadingState());
    int indexV = videoControllerList.indexWhere((element) =>
        element.initialVideoId.trim() ==
        event.videoPlayerController.initialVideoId.trim());
    if (indexV == -1) return;
