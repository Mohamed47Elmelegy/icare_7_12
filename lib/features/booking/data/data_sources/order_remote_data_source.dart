...
abstract class OrderRemoteDataSourceImpl {
  ...
  Future<OrderResponse> updateOrder({required Map<String,dynamic> data,File? fileR});
...
class OrderRemoteDataSource implements OrderRemoteDataSourceImpl {
  ...
  Future<List<OrderModel>> getAllOrder() async {
    var response = await client.get(Uri.parse("${ApiUrl.FETCH_ALL_ORDERS}/${Util.getUserID()}"), headers: ApiUrl.headerAuth);
  ...
  Future<OrderResponse> addOrder({required Map<String,dynamic> data}) async {
    ...
    if (decodedData['success']==true) {
      ...
      return OrderResponse(state: true, msg: decodedData['message']??"", orderID: "");
    } else {
      String msg = decodedData['message'].toString().contains("busy")?translate("icare.nurse_is_busy"):decodedData['message'].toString();
      return OrderResponse(state: false, msg: msg, orderID: "");
    }
  }

  @override
  Future<OrderResponse> updateOrder({required Map<String,dynamic> data,File? fileR}) async {
    var request = http.MultipartRequest('POST', Uri.parse(ApiUrl.UPDATE_ORDER_STATUS));
    var headers = ApiUrl.headerAuth;
    if(data['status']!=null)request.fields['status'] = data['status'].toString();
    if(data['booking_id']!=null)request.fields['booking_id'] = data['booking_id'].toString();
    ...
    if (res.body.toString().contains("true")) {
      ...
      return OrderResponse(state: false, msg: translate("icare.must_edit"), orderID: "");
    }else{
      return OrderResponse(state: false, msg: translate("toast.oops"), orderID: "");
    }
  }
  ...
  static Future<bool> giveAccessEditProfile() async{
    try{
      final response = await http.post(Uri.parse(ApiUrl.GIVE_ACCESS),
        headers: {
  ...
  @override
  Future<OrderResponse> sendRequest({required Map<String,dynamic> data}) async {
    try{
  ...
  static Future<dynamic> acceptOffer({required Map<String,dynamic> data}) async{
    try{
    ...
    var decodedData = jsonDecode(response.body);
    String msg = (decodedData['message']==null?translate('toast.oops'):
    (decodedData['message'].toString().contains('nurse is busy')?translate('icare.nurse_is_busy'):decodedData['message'].toString()));
