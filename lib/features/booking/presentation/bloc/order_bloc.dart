...
class BookingBloc extends Bloc<BookingEvent,BookingState>{
  ...
  collectNewBookingData(CollectNewBookingDataOrder event,emit){
    ...
    if(patientDocument!=null)patientDocument =  File(event.bookingData['patientDocument']);
    if(patientDocument2!=null)patientDocument2 =  File(event.bookingData['patientDocument2']);
    emit(OrderSuccessfullyState());
  }
  ...
  List<Booking> getCurrentOrdersByType(){
    return bookingList.where((element) => OrderModel.getStatusViewCheck(element.status.toString()) == currentOrdersType).toList();
  }
  ...
  addNewOrder(AddOrderEvent event,emit)async{
    ...
    try{
      var orderData = collectOrderData(payment: event.payment,orderData: event.orderData);
  ...
  updateBookingServiceList(UpdateBookingServiceListEvent event,emit){
    emit(OrderLoadingState());
    int index = orderServiceList.indexWhere((element) => element.id==event.service.id);
    if(index==-1){
  ...
  bool checkIfHasCurrentBooking(){
    int checkIndex = bookingList.indexWhere((element) => OrderModel.getStatusViewCheck(element.status.toString())==ORDER_STATUS.ONGOING);
    return checkIndex != -1;
  }
  ...
  bool checkIfExistBookingOngoingAndNurseCanNotEdit(){
    int checkIndex = bookingList.indexWhere((element) => OrderModel.getStatusViewCheck(element.status.toString())==ORDER_STATUS.ONGOING && element.nurseCanEditPatientProfile==false);
    return checkIndex != -1;
  }
  ...
  updateRequestForm(UpdateRequestFormDataEvent event,emit){
    ...
    if(event.data['full_name']!=null)formData['full_name'] = event.data['full_name'].toString();
    if(event.data['phone']!=null)formData['phone'] = event.data['phone'].toString();
    if(event.data['main_medical']!=null)formData['main_medical'] = event.data['main_medical'].toString();
    if(event.data['national_id']!=null)formData['national_id'] = event.data['national_id'].toString();
    if(event.data['more_info']!=null)formData['more_info'] = event.data['more_info'].toString();
    

    if(event.data['range_number']!=null)rangeNumber = event.data['range_number'].toString();
    if(event.data['date_val']!=null)dateVal = event.data['date_val'].toString();
    if(event.data['movment_level']!=null)movmentLevel = event.data['movment_level'].toString();
    if(event.data['need_to']!=null)needTO = event.data['need_to'].toString();
    if(event.data['more_need']!=null)moreNeed = event.data['more_need'].toString();
  ...
  validateForm(emit){
    ...
    if(formData['full_name']==null){
      emit(SendBookingRequestFialedState(msg:  "${translate('toast.empty')} [ ${translate('signup.full_name')} ]"));
      return false;
    }
    if(formData['phone']==null){
      emit(SendBookingRequestFialedState(msg: "${translate('toast.empty')} [ ${translate('signup.phone')} ]"));
      return false;
    }
    if(formData['main_medical']==null){
      emit(SendBookingRequestFialedState(msg: "${translate('toast.empty')} [ ${translate('signup.main_medical')} ]"));
      return false;
    }
    if(formData['national_id']==null){
      emit(SendBookingRequestFialedState(msg: "${translate('toast.empty')} [ ${translate('signup.national_id')} ]"));
    }
    if(formData['more_info']==null){
      emit(SendBookingRequestFialedState(msg: "${translate('toast.empty')} [ ${translate('icare.more_info')} ]"));
      return false;
    }
    return true;
  }
