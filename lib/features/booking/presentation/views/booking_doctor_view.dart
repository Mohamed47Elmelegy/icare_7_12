...
/// Doctor view for booking details - shows patient information
class BookingDoctorView extends StatelessWidget {
  final Booking item;

  const BookingDoctorView({super.key, required this.item});

  @override
  Widget build(BuildContext context) {
    // Get doctor entity for actions
    final list = DoctorBloc.get(context).doctorsList;
    final index = list.indexWhere((e) => item.nurseID == e.id);
    final orderDoctor = index != -1 ? list[index] : null;

    // Use singleton cubit to preserve cache (reusing BookingNurseCubit for now)
