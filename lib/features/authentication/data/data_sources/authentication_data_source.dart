...
class AuthServiceRemoteDataSource implements AuthServiceRemoteDataSourceImpl {
  ...
  Future<AuthResponse> loginUser(Map<String, dynamic> userData) async {
    try{
      ...
      }else if(response.body.contains("Unauthorized")||response.body.contains("user not found")){
  ...
  @override
  Future<AuthResponse> registerUser(Map<String, dynamic> userData,{bool social = false,}) async {
    try{
      ...
      if(userData['user_type']==null|| userData['user_type']=="") userData['user_type']="customer";
      ...
      request.fields['user_type'] = userData['user_type']==null?"customer": userData['user_type'].toString().trim().replaceAll("null", "customer");
      if(userData['city'] != null)request.fields['city'] = userData['city'];
      if(userData['governorate'] != null)request.fields['governorate'] = userData['governorate'];
      if(userData['address'] != null)request.fields['address'] = userData['address'];
      if(userData['latitude'] != null)request.fields['latitude'] = userData['latitude'].toString();
      if(userData['longitude'] != null)request.fields['longitude'] = userData['longitude'].toString();
      if(userData['country_code'] != null)request.fields['country_code'] = userData['country_code'];
      if(userData['status'] != null)request.fields['status'] = userData['status'];
      if(userData['password']!=null)request.fields['password'] = userData['password'];
      if(userData['is_male']!=null)request.fields['is_male'] = userData['is_male'];


      if(userData['languages']!=null)request.fields['languages'] = userData['languages'];
      if(userData['education']!=null)request.fields['education'] = userData['education'];
      if(userData['publications']!=null)request.fields['publications'] = userData['publications'];
      if(userData['courses']!=null)request.fields['courses'] = userData['courses'];

      if(userData['license']!=null){
        var file = await http.MultipartFile.fromPath('license_practice', userData['license'].path);
        request.files.add(file);
      }
      if(userData['certificate']!=null){
        var file = await http.MultipartFile.fromPath('graduation_certificate', userData['certificate'].path);
        request.files.add(file);
      }
      if(userData['nurseID']!=null){
        var file = await http.MultipartFile.fromPath('identification_card', userData['nurseID'].path);
        request.files.add(file);
      }
      if(userData['associationCard']!=null){
        var file = await http.MultipartFile.fromPath('association_card', userData['associationCard'].path);
        request.files.add(file);
      }
      ...
      if(userData['avatar']!=null){
        var file = await http.MultipartFile.fromPath('avatar', userData['avatar'].path);
        request.files.add(file);
      }
