...
class UserServiceRemoteDataSource implements UserServiceRemoteDataSourceImpl {
  ...
  Future<UserServiceModel> getUserData() async{
    var response = await client.get(Uri.parse("${ApiUrl.USER_PROFILE_DATA}/${ApiUrl.headerAuth['ID']}"),
  ...
  Future<UserServiceModel> updateUserProfile({required Map<String, dynamic> userData}) async {
    var res =  const UserServiceModel(userId: 0,userName: '',email: '',phoneNumber: "",userType: "",address: "",image: "");
    ...
    if(userData['avatar']!=null)await updateImg(imgPath: userData['avatar']);
    if(userData['profile']!=null){
      var body = {
        ...
        if(userData['governorate']!=null)'governorate': userData['governorate'],
        if(userData['city']!=null)'city': userData['city'],
        if(userData['postal_code']!=null)'postal_code': userData['postal_code'],
      ...
      var response = await client.post(Uri.parse("${ApiUrl.UPDATE_USER_PROFILE}/${ApiUrl.headerAuth['ID']}"),
      ...
      if(decodedData['status']==true){
        ...
        return const UserServiceModel(userId: 0,userName: '',email: '',phoneNumber: "",userType: "",address: "",image: "");
      }
    }
    return res ;
  }

  Future<UserServiceModel> updatePatientData({required Map<String, dynamic> userData}) async {
    var data = {
  ...
  @override
  Future<bool> updateProfileStatus({required Map<String, dynamic> userData}) async {
    var response = await client.post(Uri.parse("${ApiUrl.UPDATE_USER_PROFILE_STATUS}/${userData['status'] ?? 'offline'}/${Util.getUserID()}"),
        headers: ApiUrl.headerAuth);
  ...
  static Future<UserServiceModel> updateNurseOptionsValue({required Map<String, dynamic> userData}) async {
    var data = {
      'user_id': Util.getUserID(),
      if(userData['languages']!=null)'languages': userData['languages'],
      if(userData['education']!=null)'education': userData['education'],
      if(userData['publications']!=null)'publications': userData['publications'],
      if(userData['courses']!=null)'courses': userData['courses'],
      if(userData['emergency_contacts']!=null)'emergency_contacts': userData['emergency_contacts'],
  ...
  static Future<List<ServicesModel>> getAllServicesList() async {
    try{
      var response = await http.get(Uri.parse(ApiUrl.SERVICES),
        headers: ApiUrl.headerAuth);
      // debugPrint("getAllServicesList: ${response.body}");
      var decodedData = jsonDecode(response.body);
      if(decodedData['status']==true){
  ...
  static Future<bool> updateImg({required String imgPath})async{
    try{
      String url = "${ApiUrl.UPDATE_USER_PROFILE_IMG}${Util.getUserID()}/profile";
  ...
  static Future<UserServiceModel> getUserFullData(String userID) async{
    var response = await http.get(Uri.parse("${ApiUrl.USER_PROFILE_DATA}/$userID"),
        headers: ApiUrl.headerAuth);
  ...
  static Future<bool> sendNotification({required Map<String,dynamic> data})async{
    try{
      var request = http.MultipartRequest('POST', Uri.parse(ApiUrl.SEND_NOTIFICATION));
      var headers = ApiUrl.headerAuth;
      request.headers.addAll(headers);
