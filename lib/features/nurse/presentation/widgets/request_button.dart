          ...
          child: BlocBuilder<BookingBloc,BookingState>(
            builder: (ctx,state){
              ...
              return CustomButton(
                ...
                width: 180.w,
                widget: CustomText(
                  text: userHasSelectedService? translate("nurse.request") : translate("icare.select_service_first"),
                ...
                onPressed:()async{
                  ...
                  if(!userHasSelectedService){
                    return SnackBarBuilder.showFeedBackMessage(context, translate("icare.select_service_first"),DMUtil.getPC());
                  }
                  
                  final res = await Util.pushPage(MapScreen(isSet: true, title: translate('profile.confirm_current_location')), context);
                  if(res!=null && res is  LocationMapEntity){
                    bookingBloc.add(AddOrderEvent(context: context, payment: const PaymentOption(paymentEnum: PaymentEnum.CASH), orderData: {
