...
class NotificationsUtils{
  ...
  static void pushNotificationListener(BuildContext context)async{
     if(!Platform.isIOS){
      if(Util.isCustomer())await FirebaseMessaging.instance.subscribeToTopic('patient'); 
      if(Util.isNurse())await FirebaseMessaging.instance.subscribeToTopic('nurses'); 
      if(Util.isAssistant())await FirebaseMessaging.instance.subscribeToTopic('assistant'); 
      if(!Util.checkUser())await FirebaseMessaging.instance.subscribeToTopic('discover_users'); 
     }

      FirebaseMessaging.onMessage.listen((event) {
        SetNotification.showFlutterNotification(RemoteMessage(notification: event.notification!));

        if(event.notification==null || event.notification!.body==null)return;
  ...
  static checkNotification(context,RemoteMessage event){
    try{
      if(event.notification!.body!.contains("request") || event.notification!.body!.contains("حجز") || event.notification!.body!.contains("الحجز")){
      ...
      if(event.notification!.body!.contains("صلاحية") || event.notification!.body!.contains("permission")){
